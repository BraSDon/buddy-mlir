// RUN: not buddy-opt %s \
// RUN:     --lower-gemmini="tile_i=100 tile_j=100 tile_k=100" 2>&1 | \
// RUN: FileCheck %s

// Test that oversized tiles produce a diagnostic error.
...
func.func @main() -> i8 {
  ...
  // CHECK: tile sizes exceed scratchpad/accumulator capacity
